...
void test_lifepo4_profile_requires_specific_followup_tuning_but_can_count_coulombs() {
    const auto p = makeProfile(BatteryChemistry::LiFePO4, 100.0);
    TEST_ASSERT_TRUE(p.socEnabled);
    TEST_ASSERT_DOUBLE_WITHIN(0.001, 0.99, p.chargeEfficiency);
}
...
int main(int argc, char** argv) {
    ...
    RUN_TEST(test_lifepo4_profile_requires_specific_followup_tuning_but_can_count_coulombs);
    return UNITY_END();
}
